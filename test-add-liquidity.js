const { createWalletClient, http, createPublicClient, parseAbi, parseEther } = require('viem')
...
async function testAddLiquidity() {
  ...
  const managerAddress = '0xdc64a140Aa3E981100a9becA4E685f962f0cF6C9'
  ...
  try {
    ...
    // 构建回调数据
    const data = `0x${wethAddress.slice(2)}${usdcAddress.slice(2)}`
    ...
    try {
      ...
      console.log(`无法读取池子流动性: ${err.message}`)
    }
    ...
    try {
      ...
      console.log(`无法读取池子状态: ${err.message}`)
    }
