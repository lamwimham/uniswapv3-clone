...
async function checkStatus() {
  ...
  try {
    ...
    console.log("=== 账户余额 ===")
    const balance = await publicClient.getBalance({ address: defaultAccount })
    console.log(`账户: ${defaultAccount}`)
    ...
    try {
      ...
      console.log(`WETH 余额: 无法读取 - ${(err as Error).message}`)
    }
    ...
    try {
      ...
      console.log(`USDC 余额: 无法读取 - ${(err as Error).message}\n`)
    }

    // 检查 Factory 合约
    console.log("=== Factory 合约状态 ===")
    ...
    console.log("\n=== 总结 ===")
    console.log("1. 账户有足够的ETH余额")
    console.log("2. 合约已成功部署")
    console.log("3. 已创建池子，可以添加流动性")
    console.log("4. 前端现在应该能够正确显示添加流动性的表格")
